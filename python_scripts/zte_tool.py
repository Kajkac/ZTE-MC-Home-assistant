...
class zteRouter:
    ...
    def get_AD(self):
        ...
        header = {"Referer": self.referer}
        rd_response = s.get(self.referer + "goform/goform_get_cmd_process?isTest=false&cmd=RD", headers=header,
                            verify=False)
...
# in case one needs to debug outgoing or incoming sms, this will come in handy
    ...
    time.sleep(3)
    result = zteInstance.connect_data()

elif ha_select == 10:
    time.sleep(3)
    result = zteInstance.disconnect_data()
